...
constructor(
    ...
    fun bind(state: LoadingState) {
        isVisible = state != LoadingState.Success
        bindEmptyView(state)
        bindErrorView(state)
...
sealed interface LoadingState {
    ...
    data class Empty(val message: Text? = null) : LoadingState
    data class Error(val message: Text? = null, val retryable: Boolean = true) : LoadingState
}
