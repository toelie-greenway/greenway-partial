...
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
  ...
  init {
    context.theme
      ...
      .apply {
        try {
          val textAppearance =
            getResourceId(R.styleable.GreenWayLargeDropdownTextInputView_android_textAppearance, 0)
          setTextAppearance(textAppearance)

          val label = getString(R.styleable.GreenWayLargeDropdownTextInputView_android_label) ?: ""
          if (label.isNotEmpty()) {
            setLabel(label)
          }

          val text = getString(R.styleable.GreenWayLargeDropdownTextInputView_android_text) ?: ""
          setText(text)
        } finally {
          recycle()
        }
  ...
  private fun showDropdown() {
    ...
    _items.forEachIndexed { index, item -> menu.menu.add(0, item.itemId.toInt(), index, item.displayText) }
  ...
  fun setLoadingState(state: LoadingState<T>) {
    _loadingState = state
